...
@inproceedings{Bolme2010Visual,
  title={Visual object tracking_single using adaptive correlation filters},
...
from utils.utils import gaussian2d_labels, cos_window
from tracking_single.base import BaseCF

import os

class MOSSE(Base_Tracking):
    ...
    def initiate(self, image, detection):
        if len(image.shape) != 2:
            ...
            first_frame = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    def predict(self, image):
        ...
        return np.array([self._center[0]-self.w/2, self._center[1]-self.h/2, self.w, self.h])
    ...
    def update(self, image, detection):
        ...
        current_frame = image.astype(np.float32)/255

        self._center = (detection.roi[0] + detection.roi[2] / 2,
                        detection.roi[1] + detection.roi[3] / 2)
        self.w, self.h = detection.roi[2], detection.roi[3]
